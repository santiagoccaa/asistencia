...
def insertar_asistencia(dato):
	try: 
		...
		if dato != "":
			...
			cursor.execute("SELECT nombre,cuenta FROM `empleados` WHERE cedula={}".format(dato))
...
def insertar_salida(cc):
	try:
		...
		if cc != "":
			...
			sql="SELECT hora_ingreso, nombre FROM asistencia WHERE cedula='{}'".format(cc)
			...
			sql2="SELECT hora_salida FROM asistencia WHERE cedula='{}'".format(cc)
			...
			sql="UPDATE asistencia SET tiempo_laborado = %s WHERE cedula='{}'".format(cc)
			trabajado=resultado
